<script>
    ...
    *
    * @property {{ sanitizeWithVideo: function }}   [DOMPurify] The DOMPurify export from `#runtime/dompurify`.
    * Sanitizes content client side. Note: TinyMCE already does essential `<script>` sanitization, so this is just an
    * extra option that is available as an extra precaution.
    *
